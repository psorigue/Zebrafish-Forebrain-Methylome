...
path_write <- file.path("//files1.igc.gulbenkian.pt/folders/ANB/Pol/Methylome/reproducibility/site-level/", mod)
dir.create(path_write, recursive = T, showWarnings = F)

dir <- file.path(
  "//files1.igc.gulbenkian.pt/folders/ANB/Pol/Methylome/Data_methylation/datasets_proportions/ch_ss/"
)

files <- list.files(
  path = dir,
  pattern = paste0("\\_", mod, "_mpct.txt$"),
  full.names = TRUE
)
